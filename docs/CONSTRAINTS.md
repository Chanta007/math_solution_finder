...
- `PreToolUse` (`plugins/mindcoachlabs/scripts/pre-tool-guard.py`): matches `Write|Edit`, blocks (`exit 2`) when the target is a source file and neither `docs/plans/.active-plan.md` nor `docs/plans/.active-execution.md` exists. Carve-outs (always allowed): `docs/`, `.claude/`, `CLAUDE.md`, `.git/`, `.active-plan.md`, `.active-execution.md`. When the file path can't be parsed, falls back to `additionalContext` nudge instead of blocking.
